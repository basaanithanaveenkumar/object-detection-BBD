...
def visualize_results(stats):
    ...
    df_classes = pd.DataFrame.from_dict(
        stats["class_counts"], orient="index", columns=["count"]
    ...
    plt.subplot(2, 2, 1)
    ax = sns.barplot(x=df_classes.index, y="count", data=df_classes, palette="viridis")
    ...
    if stats["global_attributes"]:
        ...
        attr_data = pd.DataFrame.from_dict(
            stats["global_attributes"][attr_name], orient="index", columns=["count"]
        ).sort_values("count", ascending=False)
    ...
    if stats["height_width"]:
        ...
        hw_data = pd.DataFrame(
            stats["height_width"][sample_class], columns=["height", "width"]
        )
        sns.scatterplot(data=hw_data, x="width", y="height", alpha=0.5)
